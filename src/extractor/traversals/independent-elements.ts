...
export class IndependentElementProcessor extends BasicElementProcessor {
  ...
  public constructor(f: Flow, k: Knowledge, queryObject2fields: Map<string, string[]>) {
    super(k);
    this.f = f;
  ...
  private processConstants(flowConstants: FlowConstant[]): void {
    ...
    for (const e of flowConstants) {
      ...
      this.knowledge.builder
        .getMainClass()
        .registerConstant(e)
        .registerType(apexDataType.getResult())
        .registerRightHand(rightHand);
    }
  }
  ...
  private processDynamicChoiceSets(flowDyns: FlowDynamicChoiceSet[]): void {
    ...
    for (const e of flowDyns) {
      ...
      if (e.collectionReference) {
        ...
        this.knowledge.builder
          .getMainClass()
          .registerVariableBasedOnFlowElement(e)
          .registerType('String')
  ...
  private processStages(flowStages: FlowStage[]): void {
    ...
    const currentStageApexVariable = this.knowledge.builder
      .getMainClass()
  ...
  private processTextTemplates(flowTextTemplates: FlowTextTemplate[]): void {
    ...
    for (const e of flowTextTemplates) {
      const esc = "'" + e.text[0].replaceAll('\\', '\\\\').replaceAll("'", "\\'").replaceAll('\n', '\\n') + "'";
      this.knowledge.builder
        .getMainClass()
        .registerVariableBasedOnFlowElement(e)
        .registerType('String')
        .registerRightHand(esc);
    }
  }
  ...
  private processDynamicChoiceSetsForPicklistObject(e: FlowDynamicChoiceSet, i: number): void {
    ...
    const name: string = e.name[0];
    const apexMethod = this.knowledge.builder
      .getMainClass()
      .registerMethod(e, METHOD_PREFIXES.METHOD_PREFIX_POPULATE, name);
    ...
    if (e.sortOrder) {
      ...
      if (sortOrder === 'Desc') {
        ...
        // ${name}[i] = ${name}[n-i-1];
        // ${name}[n-i-1] = s;`;
        const apexSectionDesc = new ApexSection().addSections([
          apexAssignmentOne,
          apexAssignmentTwo,
          apexAssignmentThree,
        ]);
        const apexForStatement = apexFor()
          .lt(varN + '/2')
          .set(apexSectionDesc);
    ...
    const apexVariablePickList = this.knowledge.builder
      .getMainClass()
      .registerVariable(varPickListVal)
      .registerType('String');
    const apexVariableFr = this.knowledge.builder
      .getMainClass()
      .registerVariable(varFr)
      .registerType('Schema.DescribeFieldResult');
    const apexVariablePles = this.knowledge.builder
      .getMainClass()
      .registerVariable(varPles)
      .registerType('Schema.PicklistEntry')
      .registerIsCollection();
    ...
    const apexForStatement = apexFor()
      .item('Schema.PicklistEntry')
      .itemInstance(varPickListVal)
      .items(varPles)
      .set(apexSectionAdd);

    const apexAssignFirstLine = new ApexSectionLiteral(
      `Schema.DescribeFieldResult ${varFr} = ${obj}.${picklistField}.getDescribe();`
    ).registerVariable(apexVariableFr);
    const apexAssignSecondLine = new ApexSectionLiteral(
      'List<Schema.PicklistEntry> ' + varPles + ' = ' + varFr + '.getPicklistValues();'
