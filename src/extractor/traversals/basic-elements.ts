...
export class BasicElementProcessor {
  ...
  public constructor(knowledge: Knowledge) {
  ...
  protected prepare4Retrieval(e: FlowNode, p: string): string {
    ...
    try {
      ...
      const err = error ? (error as Error).stack ?? '' : 'unknown';
      throw new Error(`The Flow element ${e.name[0]} is apparently not properly connected. Error: ${err}`);
    }
