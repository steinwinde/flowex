...
export class SharedUse {
  ...
  private processVariable(variableName: string, methodsThatUseVariable: SimpleMethod[]): void {
    if (!SharedUse.hasExactlyOneCallingMethod(methodsThatUseVariable)) {
    ...
    if (isUsedInConstructor) {
      ...
      const topLevelMethod = SharedUse.callingMethodIsMethodCalledByConstructor(methodsThatUseVariable);
    ...
    SharedUse.markMethodsAndVariables(methodsThatUseVariable, variableName);
  }

  private static markMethodsAndVariables(methodsThatUseVariable: SimpleMethod[], variableName: string): void {
   ...
   * @returns
   */
  private isAssignedButCanNotBeReturned(
    variableName: string,
    methodsThatUseVariable: SimpleMethod[],
    methodsAlreadyChecked: Set<string>
  ): boolean {
    const methodsAssigningVariable = methodsThatUseVariable.filter((m) =>
      m.variableInfos.some((v) => v.v.s === variableName && v.u)
    );
    for (const method of methodsAssigningVariable) {
      ...
      if (
        method.callingMethods.length > 0 &&
        this.isAssignedButCanNotBeReturned(variableName, method.callingMethods, methodsAlreadyChecked)
      ) {
        return true;
      }
    }
   ...
   * @returns
   */
  private static hasExactlyOneCallingMethod(methodsThatUseVariable: SimpleMethod[]): boolean {
    const methodsCallingMethodsUseVariable = methodsThatUseVariable
      .flatMap((m) => m.callingMethods)
      .filter((method) => !methodsThatUseVariable.includes(method));
    return methodsCallingMethodsUseVariable.length !== 1;
  }

  private static callingMethodIsMethodCalledByConstructor(
    methodsThatUseVariable: SimpleMethod[]
  ...
  private getMapVariableToMethods(): Map<string, SimpleMethod[]> {
    const m = new Map<string, SimpleMethod[]>();
    for (const method of this.methods) {
