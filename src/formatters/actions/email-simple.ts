...
export default class EmailSimple implements BasicAction {
  ...
  public constructor(actionName: string, inputParams: Map<string, string>) {
    this.body =
      `Messaging.reserveSingleEmailCapacity(1);${NL}` +
      `Messaging.SingleEmailMessage mail = new Messaging.SingleEmailMessage();${NL}`;

    if (
      inputParams.get('addThreadingTokenToBody') === 'true' ||
      inputParams.get('addThreadingTokenToSubject') === 'true'
    ) {
      this.body +=
        `Id relatedId = Id.valueOf(relatedRecordId);${NL}` +
    ...
    this.body +=
      inputParams.get('logEmailOnSend') === 'true'
    ...
    if (inputParams.get('senderAddress')) {
      this.body +=
        `OrgWideEmailAddress[] owea = [SELECT Id FROM OrgWideEmailAddress WHERE Address =: senderAddress];${NL}` +
        `if(owea.size() > 0) {${NL}` +
    ...
    if (inputParams.get('emailBody')) {
      if (inputParams.get('addThreadingTokenToBody') === 'true') {
        this.body +=
          inputParams.get('sendRichBody') === 'true'
