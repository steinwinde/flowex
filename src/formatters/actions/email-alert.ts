...
export default class EmailAlert implements BasicAction {
  private body: string;
  private parameterTypes = new Map<string, string>([['SObjectRowId', 'Id']]);

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  public constructor(name: string, actionName: string, inputParams: Map<string, string>) {
    ...
    const soqlStatement = soql()
      ...
      .build();
    this.body =
      `// TODO: Provide DeveloperName of the email template configured on ${uniqueName}${NL}` +
      ...
      `mail.setTreatTargetObjectAsRecipient(false);${NL}` +
