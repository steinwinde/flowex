...
export function getScreens(flowElem: FlowScreen): ApexSection {
  ...
  const body = `UI "${flowElem.name[0]}" begin (all assignments are stubs)`;
  ...
  if (flowElem.fields) {
    for (const field of flowElem.fields) {
      ...
      if (type === 'DropdownBox') {
        ...
        const dyn: FlowDynamicChoiceSet | undefined = knowledge.name2node.get(name)
          ?.flowElement as FlowDynamicChoiceSet;
        if (dyn === undefined) {
        ...
        field.storeOutputAutomatically[0] === 'true'
      ) {
        // anything here?!
      }
...
function getStub(field: FlowScreenField): string {
  if (
    field.fieldType[0] === 'DropdownBox' &&
    field.choiceReferences &&
    knowledge.choices.has(field.choiceReferences[0])
  ) {
