...
function translateAssignmentItem(ai: FlowAssignmentItem, var2type: Map<string, Variable>): ApexSection {
  ...
  switch (op) {
    ...
    case 'RemoveUncommon': {
      ...
      const apexIfCondition = apexIfConditionFromString(condition, [
        apexVariableRightHand,
        apexVariableLeftHand,
        apexVariableI,
      ]);
    ...
    case 'RemovePosition': {
      const apexVariableLeftHand = apexVariableFromResourceName(leftHand.v);
      return new ApexSectionLiteral(`${leftHand.v}.remove(${Number.parseInt(rightHand.v, 10)});`).registerVariable(
        apexVariableLeftHand
      );
      // confirmed working with Text List (Test: "TextAssignments")
...
function getAdd(
  leftHand: MyFlowElementReferenceOrValue,
  rightHand: MyFlowElementReferenceOrValue,
...
function getSubtract(leftHand: MyFlowElementReferenceOrValue, rightHand: MyFlowElementReferenceOrValue): ApexSection {
  ...
  if (leftHand.t === 'Date') {
    const i = Number.parseInt(rightHand.v, 10);
    const apexSectionLiteral = new ApexSectionLiteral(`${leftHand.v}.addDays(-${i});`).registerVariable(
      apexVariableLeftHand
    );
  ...
  // const apexVariableRightHand = apexVariableFromResourceName(rightHand.v);
  const apexSectionLiteral = new ApexSectionLiteral(`${leftHand.v} -= ${rightHand.v};`).registerVariable(
    apexVariableLeftHand
  );
...
function getRemoveFirst(leftHand: MyFlowElementReferenceOrValue, rightHand: MyFlowElementReferenceOrValue): ApexFor {
  // Collection only (picklist, multi-picklist, $Flow.CurrentRecord)
  const condition = getCondition(leftHand, rightHand);
  const apexSectionLiteral = new ApexSectionLiteral(`${leftHand.v}.remove(i);`).registerVariables([
    apexVariableFromResourceName(leftHand.v),
    new ApexVariable('i').registerType('Integer'),
  ]);
  const apexSectionLiteralBreak = new ApexSectionLiteral('break;');
...
function getRemoveAfterFirst(
  ...
  const apexSectionLiteral = new ApexSectionLiteral(bodyInner).registerVariable(
    apexVariableFromResourceName(leftHand.v)
  );
