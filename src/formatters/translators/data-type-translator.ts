...
export class ApexDataType {
  ...
  private constructor(dataType: FlowDataType, scale?: number, apexClass?: string, objectType?: string) {
    switch (dataType) {
      ...
      default: {
        const dataTypeString = dataType as string;
        throw new Error('Unknown data type: ' + dataTypeString);
      }
    }
  }

  public static fromFlowConstant(flowConstant: FlowConstant): ApexDataType {
    // a FlowConstant has no scale information, just a value
    const value = flowConstant.value[0];
    const numberValue = value.numberValue?.[0] !== undefined ? value.numberValue[0] : undefined;
  ...
  public static fromFlowVariableBase(flowVariableBase: FlowVariableBase): ApexDataType {
    return new ApexDataType(
      flowVariableBase.dataType[0],
      flowVariableBase.scale ? flowVariableBase.scale[0] : undefined
    );
  }

  public static fromFlowVariable(flowVariable: FlowVariable): ApexDataType {
  ...
  public static fromFlowScreenField(flowScreenField: FlowScreenField): ApexDataType {
    const fieldType = flowScreenField.fieldType[0];
    if (FLOW_SCREEN_FIELD_WITH_DATA_TYPE.has(fieldType)) {
      return new ApexDataType(
        flowScreenField.dataType[0],
        flowScreenField.scale ? flowScreenField.scale[0] : undefined
      );
    }
