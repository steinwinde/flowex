...
export class ApexSection {
  ...
  public addSection(section: ApexSection): ApexSection {
  ...
  // only extending classes can write strings to the sections
  public addSections(sections: ApexSection[]): ApexSection {
  ...
  protected getBuiltSections(): string[] {
    const sections: string[] = this.sections.map((section) => {
      if (typeof section !== 'string') {
