...
export class ApexIfCondition extends ApexSection {
    ...
    setString(body: string, apexVariables : Array<ApexVariable>): ApexIfCondition {
    ...
    setFlowRecordFilter(filterLogic: string, filters: FlowRecordFilter[], ref: string): ApexIfCondition {
        this.addStringSection(this.getFilters(filterLogic, filters, ref));
        return this;
    }

    setFlowCondition(conditionLogic: string, conditions: FlowCondition[], includeNullCheck: boolean): ApexIfCondition {
    ...
    private getFilters(filterLogic: string, filters: FlowRecordFilter[], ref: string) : string {
        let result = '';
...
export function apexIfConditionFromCollectionProcessor(collectionProcessor: FlowCollectionProcessor): ApexIfCondition {
    return new ApexIfCondition().setFlowCondition(collectionProcessor.conditionLogic[0], [collectionProcessor.conditions![0]], false);
}

export function apexIfConditionFromFlowRecordFilter(filterLogic: string, filters: FlowRecordFilter[], ref:string): ApexIfCondition {
    return new ApexIfCondition().setFlowRecordFilter(filterLogic, filters, ref);
}

export function apexIfConditionFromString(s : string, apexVariables: Array<ApexVariable>): ApexIfCondition {
