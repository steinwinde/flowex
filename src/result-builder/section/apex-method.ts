...
export class ApexMethod extends ApexSection {
  ...
  public constructor(name: string, nodeName: string, localVariables: boolean) {
    super();
    this.name = name;
  ...
  public registerCallingMethod(callingMethod: ApexMethod): void {
    if ([...this.callingMethods.keys()].some((e) => e.getName() === callingMethod.getName())) return;

    this.callingMethods.add(callingMethod);
  }

  // Use with care; only known use 2024-06-28: FlowActions
  public registerParameter(variable: ApexVariable): ApexVariable {
  ...
  public registerReturnType(returnType: string): ApexMethod {
    this.returnType = returnType;
    return this;
  }

  public registerBody(body?: ApexSection): void {
    this.body = body;
  }
  ...
  public buildCall(argOverwrite?: string): string {
  ...
  public resolveVariables(): Map<string, ApexVariableInfo> {
    if (this.body && this.localVariables) {
  ...
  public outputCallingMethods(): void {
    const callingMethodsOutput = [...this.callingMethods.values()].map((method) => method.getName()).join(', ');
    console.error(
      'method: ' +
        this.name +
        (callingMethodsOutput
          ? ', calling methods: ' + callingMethodsOutput
          : ', no calling methods found, must be constructor')
    );
  }

  // TODO: Just for debugging
  public outputVariables(): void {
    const variablesOutput = [...this.resolveVariables().values()].map((variable) => variable.toString()).join(', ');
    console.error('method: ' + this.name + (variablesOutput ? ', variables: ' + variablesOutput : ', no variables'));
  }

  public addVariablesToCallingMethods(): boolean {
    let added = false;
  ...
  private getParams(): string {
    ...
    return [...this.resolveVariables().values()]
      ...
      .map((variable) => variable.getApexVariable().getApexType() + ' ' + variable.getApexVariable().getName())
      .join(', ');
  }
  ...
  private getArguments(): string {
    ...
    const result = [...this.resolveVariables().values()]
      .filter((variable) => variable.getApexVariable().isLocalVariable())
      .map((variable) => variable.getApexVariable().getName())
      .join(', ');
    return result;
  }
}
