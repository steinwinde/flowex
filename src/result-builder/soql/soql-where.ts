...
export class SoqlWhere extends ApexSection {
  ...
  public constructor(filters: FlowRecordFilter[], filterLogic: string, apexVariables: ApexVariable[]) {
    super();
    ...
    for (const filter of filters) {
      ...
      if (oper) {
        ...
        switch (filter.operator[0]) {
          ...
          default: {
            throw new Error('Unknown operator for given context: ' + filter.operator[0]);
          }
